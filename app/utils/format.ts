...
export const formatNumber = (i: number, digits?: number) =>
  i?.toLocaleString('en-US', { maximumFractionDigits: digits })

export const toFloat = (number: string, digits: number = 2) =>
  parseFloat(number).toFixed(digits)

export const roundNumber = (number: string) =>
  parseInt(Math.round(parseFloat(number)).toString())

export const shortString = (str: string, max: number = 40, length: number = 10) =>
  str?.length > max ? str.substring(0, length) + '...' : str
...
export const customGweiFormat = (price: bigint, digits?: number) =>
  price > 20000000000n
